...
import com.company.iendo.bean.model.ChildItem;
import com.company.iendo.bean.model.ParentItem;
...
public class ExpandableAdapterView extends RecyclerView.Adapter {
    private final static int VIEW_PARENT = R.layout.item_case_model_parent;//父级布局
    private final static int VIEW_CHILD = R.layout.item_case_model_child;//子级布局
    ...
    public void onBindViewHolder(@NonNull final RecyclerView.ViewHolder holder, final int position) {
        if (holder instanceof ParentHolder) {
            ...
            LogUtils.e("province.getProvinceName()===="+province.getProvinceName());
...
//        private List<Object> objects = new ArrayList<>();
        ...
        for (int i = 0; i < provincesList.size(); i++) {
            //将每一个省的实体放进object集合里
            objects.add(provincesList.get(i));
            //判断省展开状态，如果展开了，则省下面的市也要在界面展示出来
            if (provincesList.get(i).isExpand()) {
                //将展开的省下面的市的总数赋给基数size
                size += provincesList.get(i).getCities().size();
                //将该省下面所有市放进object集合里
                objects.addAll(provincesList.get(i).getCities());
                //遍历所有市，将省名字赋给市，便于后面处理列表时知道当前市属于哪个省
                for (int j = 0; j < provincesList.get(i).getCities().size(); j++) {
                    provincesList.get(i).getCities().get(j).setProvinceName(provincesList.get(i).getProvinceName());
                }
            }
        }
